...
mod tests {
    ...
    fn embedded_catalog_is_valid() {
        ...
        // 内置 Codex 目录 = 上游基线 11 个 + UniAPI 精选模型（sync-uniapi-catalog.mjs 产物 4 合入）
        assert_eq!(sources.templates.len(), 41);
        ...
        // UniAPI 端点按 OpenAI API 规格提供 1.05M 上下文（sync 产物 4 覆盖），
        // 上游基线的 272K/872K 是 ChatGPT 后端会话窗口，不适用于 API 直连场景。
        ...
        for slug in ["gpt-5.3-codex-spark", "deepseek-v4-pro"] {
