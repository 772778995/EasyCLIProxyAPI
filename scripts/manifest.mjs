...
export async function generatePortableUpdateManifest({
	...
	const resolvedOutput = resolve(
		output ??
			join(resolvedDirectory, portableUpdateManifestName(normalizedPlatform)),
	);
	const resolvedRepository = repository ?? "772778995/EasyCLIProxyAPI";
	const resolvedGitcodeRepository = String(gitcodeRepository ?? "").trim();
	const normalizedRawTag = String(rawTag ?? "").trim();
	const tag = normalizedRawTag.startsWith("v")
		? normalizedRawTag
		: `v${normalizedRawTag}`;
	...
	for (const arch of ["amd64", "aarch64"]) {
		...
		if (!metadata.isFile() || metadata.size === 0) {
			throw new Error(
				`Portable release asset is empty or not a file: ${filename}`,
			);
		}
		const asset = {
		...
		if (resolvedGitcodeRepository) {
			asset.fallbackUrls = [
				gitcodeReleaseAttachmentUrl(resolvedGitcodeRepository, tag, filename),
			];
		}
...
async function main() {
	...
	const outputs = requestedOutput
		...
		: portableUpdateManifestNames(platform).map((name) =>
				join(directory, name),
			);
