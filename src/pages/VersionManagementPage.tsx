...
const APP_RELEASE_URL =
	"https://github.com/772778995/EasyCLIProxyAPI/releases/latest";
...
export function displayAppVersion(version: string) {
	...
	return resolvedVersion.startsWith("v")
		? resolvedVersion
		: `v${resolvedVersion}`;
}
...
export function VersionManagementPage() {
	...
	const { status: coreStatus, refreshStatus } = useCoreRuntime();
	const {
		latest,
		error: latestError,
	...
	const [versionSource, setVersionSource] =
		useState<VersionSourceSettings | null>(null);
	...
	const showInstallCompletedToast = (
		result: CoreInstallResult,
		message?: string | null,
	) => {
	...
	const loadVersionSourceSettings = async () => {
		try {
			const settings = await invoke<VersionSourceSettings>(
	...
	const updateVersionSource = async (source: VersionDownloadSource) => {
		...
		try {
			...
			resetLatest();
			showToast(
				t("kernel.versions.sourceSwitched", {
			...
			setVersionSourceError(
				t("kernel.versions.gitcodeSaveFailed", { error: String(error) }),
			);
			showToast(
				t("kernel.versions.gitcodeSaveFailed", { error: String(error) }),
				"error",
	...
	const addCustomMirror = async () => {
		...
		try {
			...
			const message = t("kernel.versions.customMirrorAddFailed", {
				error: String(error),
			});
	...
	const removeCustomMirror = async (url: string) => {
		...
		try {
			...
			const message = t("kernel.versions.customMirrorRemoveFailed", {
				error: String(error),
			});
			setVersionSourceError(message);
			showToast(message, "error");
	...
	const installVersion = async (version: string) => {
		...
		try {
			const result = await invoke<CoreInstallResult>("install_core_version", {
				version,
			});
			showInstallCompletedToast(
				result,
				t("kernel.install.completed", { version: result.version }),
			);
	...
	const openAppRelease = async () => {
		try {
			await invoke("open_external_url", {
				url: appUpdate?.releaseUrl || APP_RELEASE_URL,
			});
		} catch (error) {
			showToast(
				t("kernel.error.openUpdate", { error: String(error) }),
				"error",
			);
		}
	};
	...
	useEffect(() => {
		...
		void listen<VersionSourceSettings>(
			"version-download-source-changed",
			(event) => {
				if (disposed) return;
				setVersionSource(event.payload);
				setVersionSourceError("");
				showToast(
					t("kernel.versions.sourceAutoSwitched", {
	...
	const coreVersionStatusTone =
		installing || progress?.running
			? "info"
			: latestError
				? "error"
				: coreHasUpdate
	...
	const coreVersionStatusLabel: string | null =
		installing || progress?.running
			? cancellingInstall
				? t("kernel.install.cancelling")
				: progress?.phase
					? localizeInstallPhase(progress.phase, t)
					: t("kernel.install.inProgress")
			: latestError
				? t("kernel.update.failed")
				: coreHasUpdate
	...
	const installDialogTitle =
		progress?.running || installing
			? cancellingInstall
		...
		<section className="page management-page version-management-page">
			<section className="panel version-list">
				...
				>
					<div className="version-source-copy">
						...
						{versionSourceError ? (
							<span className="version-source-error" role="alert">
								{versionSourceError}
							</span>
						) : null}
					</div>
					<div className="version-source-control">
						<label>
							<span className="sr-only">
								{t("kernel.versions.downloadSource")}
							</span>
							<select
								...
								aria-label={t("kernel.versions.downloadSource")}
								onChange={(event) =>
									void updateVersionSource(
										event.currentTarget.value as VersionDownloadSource,
									)
								}
							>
								<option value="github">
									{t("kernel.versions.source.github")}
								</option>
								<option
									value="gitcode"
									disabled={!versionSource?.gitcodeAvailable}
								>
									{t("kernel.versions.source.gitcode")}
								</option>
								<option value="gh-proxy">
									{t("kernel.versions.source.ghProxy")}
								</option>
								<option value="gh-fast">
									{t("kernel.versions.source.ghFast")}
								</option>
						...
						<button
							...
							disabled={
								versionSourceSaving || appUpdateTask.running || installing
							}
							onClick={() => {
								setVersionSourceError("");
						...
						<div className="version-item-content">
							<div className="version-card-top">
								...
								{appVersionStatusLabel ? (
									<span
										className={`version-row-status ${appVersionStatusTone}`}
										title={appUpdateError || appVersionStatusLabel}
									>
										{appVersionStatusLabel}
									</span>
								) : null}
							...
							<dl className="version-metrics-comparison">
								<div className="version-metric-tile">
									<dt className="version-metric-label">
										{t("appUpdate.current")}
									</dt>
									<dd className="version-metric-value">{currentAppVersion}</dd>
								</div>
								<div
									className={`version-metric-tile ${appHasUpdate ? "has-update" : ""}`}
								>
									<dt className="version-metric-label">
										{t("appUpdate.latest")}
									</dt>
					...
					<article className="version-list-item core-module-card">
						<div className="version-item-content">
							<div className="version-card-top">
								...
								{coreVersionStatusLabel ? (
									<span
										className={`version-row-status ${coreVersionStatusTone}`}
										title={coreVersionStatusLabel}
									>
										{coreVersionStatusLabel}
									</span>
								) : null}
							...
							<dl className="version-metrics-comparison">
								<div className="version-metric-tile">
									<dt className="version-metric-label">
										{t("kernel.versions.current")}
									</dt>
									<dd
										className="version-metric-value"
										title={currentVersion || t("kernel.status.notInstalled")}
									>
										{currentVersion || t("kernel.status.notInstalled")}
									</dd>
								</div>
								<div
									className={`version-metric-tile ${coreHasUpdate ? "has-update" : ""}`}
								>
									<dt className="version-metric-label">
										{t("kernel.versions.latest")}
									</dt>
									<dd
										className="version-metric-value"
										title={
						...
						<div className="version-card-actions core-actions">
							...
							>
								<RefreshCw
									size={15}
									className={checkingLatest ? "spin" : ""}
									aria-hidden="true"
								/>
								<span>
									{checkingLatest
										? t("kernel.update.checking")
										: t("kernel.versions.check")}
								</span>
							</button>
							...
							<button
								...
								className={
									latestVersion &&
									(!coreInstalled || currentVersion !== latestVersion)
										? "primary-button"
										: "secondary-button"
								}
								title={
									latestVersion
										? t("kernel.versions.stopAndUpdateVersion", {
							...
							>
								...
								<span>
									{!coreInstalled
										? t("kernel.versions.installLatestMissing")
										: t("kernel.versions.installLatest")}
								</span>
							</button>
				...
				<div className="install-dialog-backdrop custom-mirror-dialog-backdrop">
					...
					>
						<div className="install-dialog-heading">
							...
							<h2 id="custom-mirror-dialog-title">
								{t("kernel.versions.customMirrorDialogTitle")}
							</h2>
						</div>
						...
						<input
							...
							onChange={(event) =>
								setCustomMirrorDraft(event.currentTarget.value)
							}
						/>
						{versionSourceError ? (
							<span className="custom-mirror-dialog-error" role="alert">
								{versionSourceError}
							</span>
						) : null}
			...
			{confirmUpdateOpen ? (
				<div className="install-dialog-backdrop app-update-dialog-backdrop">
					...
					>
						<div className="install-dialog-heading">
							<span className="install-dialog-eyebrow">
								{t("kernel.dialog.install")}
							</span>
						...
						<p className="app-update-confirm-copy">
							{t("kernel.versions.stopAndConfirmDescription", {
								version: latestVersion,
							})}
						</p>
					...
					>
						...
						>
							<span
								...
								style={
									progressKnown ? { width: `${progressPercent}%` } : undefined
								}
							/>
						</div>
						...
						<div className="install-progress-meta">
							<strong>
								{progressKnown
									...
									: t("kernel.dialog.unknownProgress")}
							</strong>
